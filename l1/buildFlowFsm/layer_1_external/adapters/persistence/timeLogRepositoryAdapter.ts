...
export function createTimeLogRepositoryAdapter(ctx: RequestContext): ITimeLogRepository {
  ...
  return {
    async getById(id: string): Promise<TimeLog | null> {
      ...
      const row = await repo.findOne({ where: { time_log_id: id } });
    ...
    async save(aggregate: TimeLog): Promise<void> {
      const repo = await getTable();
      const existing = await repo.findOne({ where: { time_log_id: aggregate.timeLogId } });
      if (existing) {
        await repo.update({ where: { time_log_id: aggregate.timeLogId }, patch: toRow(aggregate) });
      } else {
